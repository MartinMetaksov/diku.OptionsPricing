...
__global__ void
computeSingleOptionKernel(real *res, const OptionConstants *options, real *QsAll, real *QsCopyAll, real *alphasAll, const int totalCount, const int yieldCurveSize)
{
...
void computeOptions(const vector<OptionConstants> &options, const vector<Yield> &yield, real *result, bool isTest = false)
{
    // Compute indices
    int maxWidth = 0;
    int maxHeight = 0;
    ...
    computeSingleOptionKernel<<<blockCount, blockSize>>>(d_result, d_options, d_Qs, d_QsCopy, d_alphas, count, yield.size());
